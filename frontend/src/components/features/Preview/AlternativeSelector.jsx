      ...
      <CardBody>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {alternatives.map((alternative, idx) => {
            // Handle both Unsplash objects and custom objects
            const imageUrl = alternative.urls?.regular || alternative.urls?.small || alternative.url;
            const imageId = alternative.id || `img_${idx}`;
            const imageLabel = alternative.alt_description || alternative.label || `Image ${idx + 1}`;
            ...
            return (
              ...
              >
                ...
                <div className="bg-white dark:bg-gray-700 p-3">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100 text-center line-clamp-2">
                    {imageLabel}
                  </p>
                  {alternative.user && (
                    <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-1">
                      by {alternative.user.name}
                    </p>
                  )}
                </div>
